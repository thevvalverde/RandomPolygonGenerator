...
class Candidate extends ArrayList<Coordinate> {
	...
	private static SegmentMap pheromorn = null;
...
//				this.Q = getPerimeter();
				for(int i=1; i<this.size(); i++)
					pheromorn.put( new Pair<Coordinate>(this.get(i-1), this.get(i)), 1.0);
				pheromorn.put( new Pair<Coordinate>(this.get(size()-1), this.get(0)), 1.0);
				break;
	...
	*/
	...
	public Coordinate findAntPath(Coordinate i, ArrayList<Coordinate> list){
		...
		for(Coordinate k : list)
			if(pheromorn.get( new Pair<Coordinate>(i, k) ) != null)
				value += (pheromorn.get( new Pair<Coordinate>(i, k) ) * ( 1.0/euclidianDistance(i,k) ));
		for(Coordinate j : list){
			if(pheromorn.get( new Pair<Coordinate>(i, j) ) == null)
				pheromorn.put( new Pair<Coordinate>(i, j), 0.0 );
			else
				last += (pheromorn.get( new Pair<Coordinate>(i, j) ) * ( 1.0/euclidianDistance(i,j) ))
					/ value;
